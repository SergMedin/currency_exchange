...
class T(unittest.TestCase):
    ...
    def testOrderLifetimeExceeded(self):
        ...
        self.exchange.on_new_order(Order(user=User(1), type=OrderType.BUY, price=100.0, amount_initial=100.0, min_op_threshold=50.0, lifetime=60*60, creation_time=time.time() - 2 * 3600))
        self.exchange.on_new_order(Order(user=User(2), type=OrderType.BUY, price=100.0, amount_initial=100.0, min_op_threshold=50.0, lifetime=60*60, creation_time=time.time() - 0.5 * 3600))
        self.exchange.on_new_order(Order(user=User(3), type=OrderType.BUY, price=100.0, amount_initial=100.0, min_op_threshold=50.0, lifetime=60*60))
    ...
    def testOrderLifetimeIncorrectInput(self):
        ...
        with self.assertRaises(ValueError):
            self.exchange.on_new_order(Order(user=User(1), type=OrderType.BUY, price=100.0, amount_initial=100.0, min_op_threshold=50.0, lifetime=50*60*60))        